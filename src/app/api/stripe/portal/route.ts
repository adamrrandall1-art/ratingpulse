...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customerId, userEmail, userId, returnUrl } = body;
    ...
    if (!isStripeConfigured || !stripe) {
      return NextResponse.json({
        url: destinationUrl,
        isDemo: true,
        message: 'Stripe is running in demo mode. Configure STRIPE_SECRET_KEY to access live customer portal.',
      });
    }

    let targetCustomerId = customerId;

    // If customerId is not directly passed, look it up via Supabase or userEmail
    if (!targetCustomerId && (userId || userEmail)) {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        if (data?.stripe_customer_id) {
          targetCustomerId = data.stripe_customer_id;
        }
      }
    ...
    if (!targetCustomerId) {
      return NextResponse.json(
        {
          error: 'No active Stripe customer found for this account. Please upgrade to a paid plan first.',
